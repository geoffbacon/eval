## eval
[![Build Status](https://travis-ci.org/uwescience/eval.svg?branch=master)](https://travis-ci.org/uwescience/eval)
...
repository into your computer and follow the instructions at the [bottom of this page](#using-eval-as-a-template).
...
`eval/eval.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `eval.py` in directory called
`eval`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import eval as sb` in an
interactive Python session, the classes and functions defined inside of the
`eval.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the eval module, the main classes defined at the bottom of
...
Either way, you can create a `eval/data` folder in which you can
...
(typically, something like our `eval/tests/test_eval.py`). Within each
...
have one such test in `eval/tests/test_eval.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/eval`),
and type:
...
will see a message such as:
    ...
    eval/tests/test_eval.py:49: AssertionError
...
To document `eval` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/eval/blob/master/doc/_static/RTD-advanced-conf.png)

 http://eval.readthedocs.org/en/latest/
...
A `eval/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/eval) for the software. This
...
`eval` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `eval`, we use the
...
[here](https://github.com/uwescience/eval/blob/master/scripts/Figure1.ipynb).


### Using `eval` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/eval smallish
	cd smallish
...
Next, you will want to move `eval/eval.py` to be called
`smallish/smallish.py`

	git mv eval smallish
	git mv smallish/eval.py smallish/smallish.py
	git mv smallish/tests/test_eval.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `eval` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `eval` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `eval` in them. Type
the following to see where all these files are:

	git grep eval

You can replace `eval` for `smallish` fastly with:

	git grep -l 'eval' | xargs sed -i 's/eval/smallish/g'
...
Edit all the mentions of `eval` in the `eval/__init__.py` file, and in
the `eval/version.py` file as well.
